...
export default function GalleryPage() {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* ───────── Filter Buttons ───────── */}
      <section className="bg-beige-light py-8 border-b border-beige-dark/50">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              {categories.map((category) => (
                <button
                  ...
                  className={`px-5 py-2.5 text-sm font-medium tracking-wide uppercase transition-all duration-300 ${
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-primary/90 via-primary/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
                      <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                        <p className="text-gold text-xs tracking-[0.25em] uppercase mb-2 font-medium">
                          {image.category}
                        </p>
                      ...
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-14 h-14 rounded-full border-2 border-white/40 flex items-center justify-center opacity-0 group-hover:opacity-100 scale-50 group-hover:scale-100 transition-all duration-500 delay-100">
                        <ZoomIn size={22} className="text-white" />
                      </div>
                    </div>

                    {/* Gold accent corner */}
                    <div className="absolute top-0 left-0 w-0 h-0 border-t-[3px] border-l-[3px] border-gold opacity-0 group-hover:opacity-100 group-hover:w-8 group-hover:h-8 transition-all duration-500" />
                    <div className="absolute bottom-0 right-0 w-0 h-0 border-b-[3px] border-r-[3px] border-gold opacity-0 group-hover:opacity-100 group-hover:w-8 group-hover:h-8 transition-all duration-500" />
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
      ...
      <AnimatePresence>
        {selectedImage !== null && filteredImages[selectedImage] && (
          ...
          >
            ...
            <button
              onClick={closeLightbox}
              className="absolute top-6 right-6 z-50 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors group"
              aria-label="Close lightbox"
            >
              <X size={24} className="text-white group-hover:text-gold transition-colors" />
            </button>
            ...
            <button
              ...
              className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-50 w-12 h-12 rounded-full bg-white/10 hover:bg-gold/30 flex items-center justify-center transition-all group"
              aria-label="Previous image"
            >
              <ChevronLeft size={24} className="text-white group-hover:text-gold transition-colors" />
            </button>
            ...
            <button
              ...
              className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-50 w-12 h-12 rounded-full bg-white/10 hover:bg-gold/30 flex items-center justify-center transition-all group"
              aria-label="Next image"
            >
              <ChevronRight size={24} className="text-white group-hover:text-gold transition-colors" />
            </button>
