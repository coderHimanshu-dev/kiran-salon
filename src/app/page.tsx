...
function AnimatedCounter({ value, suffix }: { value: number; suffix: string }) {
  ...
  return (
    <span ref={ref} className="text-4xl font-heading font-bold text-gold">
      {count}
      {suffix}
    </span>
  );
    ...
    <div className="flex flex-col w-full">
      ...
      <section className="relative h-screen min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <FadeIn delay={0.4}>
            <h1 className="font-heading text-4xl sm:text-6xl md:text-7xl lg:text-8xl text-white font-bold mb-6 tracking-tight leading-tight">
          ...
          <FadeIn delay={0.8}>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/book" className="btn-luxury w-full sm:w-auto">
                Book Appointment
              </Link>
              <a
                ...
                className="bg-green-500 text-white font-semibold py-[14px] px-[36px] uppercase tracking-wider text-[0.85rem] hover:bg-green-600 transition-colors w-full sm:w-auto text-center"
              >
                WhatsApp Us
              </a>
              <a
        ...
        >
          <Icons.ChevronDown className="text-gold w-8 h-8 opacity-70" />
        </motion.div>
      </section>

      {/* 2. Trust/Stats Section */}
      <section className="bg-[#0F0F0F] py-20 relative border-b border-white/10">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <FadeIn direction="right">
              <div className="relative aspect-[4/5] w-full max-w-md mx-auto lg:max-w-none">
                <div className="absolute inset-0 border-2 border-gold translate-x-4 translate-y-4" />
                <Image
            ...
            <div>
              ...
              />
              <FadeIn delay={0.3}>
                <div className="space-y-6 text-salon-muted text-lg mb-8">
                  <p>
                ...
                <Link href="/about" className="btn-luxury-outline text-primary border-primary hover:text-white hover:bg-primary">
                  Read Our Full Story
                </Link>
              </FadeIn>
            </div>
            ...
            {services.slice(0, 6).map((service) => (
              <StaggerItem key={service.id} className="group cursor-pointer">
                <Link href={`/services#${service.id}`} className="block h-full">
                  ...
                  <h3 className="font-heading text-2xl font-semibold mb-3 group-hover:text-gold transition-colors">
                    {service.title}
                  </h3>
                  <p className="text-salon-muted mb-4 line-clamp-2">
                    {service.shortDescription}
                  </p>
                  <span className="text-gold uppercase tracking-wider text-sm font-semibold flex items-center gap-2 group-hover:gap-4 transition-all">
      ...
      <section className="bg-[#0F0F0F] section-padding text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <SectionHeading
            ...
            light
          />
          <StaggerContainer className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 mt-16">
            {whyChooseUs.map((item, index) => {
              const Icon = Icons[item.icon as keyof typeof Icons] as React.ElementType;
              return (
                <StaggerItem key={index} className="flex gap-4">
                  <div className="flex-shrink-0 w-12 h-12 rounded-full border border-gold/30 flex items-center justify-center bg-gold/5 text-gold">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            <SectionHeading
              ...
              align="left"
            />
            <Link href="/gallery" className="btn-luxury-outline text-primary border-primary hover:bg-primary hover:text-white shrink-0 self-start md:self-auto">
              View Full Gallery
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <StaggerContainer className="grid md:grid-cols-3 gap-8 mt-12">
            {testimonials.slice(0, 3).map((testimonial, index) => (
              <StaggerItem
                key={index}
                className="p-8 border border-gray-100 shadow-soft relative"
              >
                ...
                <p className="text-salon-muted italic mb-8 leading-relaxed">
                  "{testimonial.review}"
                </p>
          ...
          <div className="text-center mt-12">
            <Link href="/testimonials" className="text-primary font-semibold hover:text-gold transition-colors underline underline-offset-4">
              Read All Reviews
            </Link>
          </div>
        </div>
